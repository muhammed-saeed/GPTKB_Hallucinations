...
def _render_elicitation(
    ...
    """
    Renders the elicitation prompt. In topic-mode, templates can use {{ root_subject }}
    to constrain to the seed's topic (e.g., 'Game of Thrones', 'Ancient Babylon').
    """
    ...
    txt = tpl.render(
        ...
        hint=max_facts_hint,
        # topic-mode anchor
        root_subject=(root_subject or subject_name),
        topic=topic,
        domain=domain,
    )
    return txt
...
def _render_ner(
    ...
    """
    Renders the NER prompt. In topic-mode, templates can use {{ root_subject }}
    to classify phrases only if they belong to that topic.
    """
...
def _sanitize_prompt_for_llm_contract(prompt: str) -> str:
    """
    Removes role markers; avoids adding extra symbols or arrows.
    """
    lines = []
...
# ----- Salvage helpers (no regex, fast) -----

def _salvage_facts_from_raw(raw: str) -> List[dict]:
    """
    Greedy salvage of objects from `"facts": [ {...}, {...}, ... ]` in possibly
    truncated JSON. Tolerates missing closing ]/} and trailing commas.
    """
...
def _loose_json_from_raw(raw: str) -> dict:
    """
    Robust JSON extraction from _raw:
    1) json.loads
    2) strip code fences and retry
    3) light unescape and retry
    4) first balanced {...}
    5) salvage `"facts":[...]` items
    """
    ...
    # 5) SALVAGE facts array into a valid object
...
def main():
    ap = argparse.ArgumentParser(description="Minimal, synchronous crawler (topic-aware prompts supported).")
    ...
    ap.add_argument("--topic", default=None, help="Optional, kept for compatibility; not required in topic-mode.")
    ...
    # NOTE: No longer enforce --topic when --domain topic.
    # Topic-mode will use the seed as {{ root_subject }} inside templates.
    ...
    for cfg in (el_cfg, ner_cfg):
        ...
        # Give OpenAI-compat + Replicate/DeepSeek some headroom by default
    ...
    while True:
        ...
        try:
            ...
            # If model client returned raw text, try to recover JSON
            raw_blob = None
            ...
            # Final salvage if still empty but we saw raw text
