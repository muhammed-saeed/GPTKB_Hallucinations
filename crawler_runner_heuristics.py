...
def _supports_reasoning_controls(cfg) -> bool:
    """Only OpenAI Responses API gpt-5* models support reasoning/text controls."""
...
def _apply_reasoning_text_overrides(cfg, effort: str | None, verbosity: str | None, stage_label: str):
    """Apply CLI overrides if supported; otherwise print a note and do nothing."""
    if not _supports_reasoning_controls(cfg):
        if effort or verbosity:
            _dbg(f"[note] {stage_label}: model '{getattr(cfg,'model','')}' "
                 f"does not support --reasoning-effort/--text-verbosity; ignoring.")
        return
    ...
    _dbg(f"[model:{stage_label}] reasoning.effort="
         f"{cfg.extra_inputs['reasoning'].get('effort')} "
...
def main():
    ...
    # NEW: reasoning/verbosity overrides (only applied to supported models, e.g., gpt-5*)
    ap.add_argument(
        "--reasoning-effort",
        choices=["minimal","low","medium","high"],
        default=None,
        help="Override reasoning.effort for supported Responses API models (gpt-5*)."
    )
    ap.add_argument(
        "--text-verbosity",
        choices=["low","medium","high"],
        default=None,
        help="Override text.verbosity for supported Responses API models (gpt-5*)."
    )
    ...
    def _apply_stage(which, cfg):
        ...
        if getattr(cfg, "use_responses_api", False):
            ...
            # Chat/completions-style models: honor standard sampling knobs
            t  = getattr(args, f"{which}_temperature")
            tp = getattr(args, f"{which}_top_p")
            tk = getattr(args, f"{which}_top_k")
            if t  is not None: cfg.temperature = t
        ...
        # >>> NEW: apply reasoning/text overrides when supported (gpt-5 Responses API) <<<
        _apply_reasoning_text_overrides(
            cfg,
