#!/usr/bin/env python3
# crawler_batch_concurrency_topic.py
...
# ===================== Retry helper for threaded path =====================

def _llm_call_with_retries(call_fn, *, max_retries: int, timeout: float, backoff: float):
    """
    call_fn signature: () -> result  (should raise on failure).
    Retries with exponential backoff (backoff * 2**attempt).
    """
    last_err = None
...
def _process_completed_elicitation_batch(meta: dict, *, paths, qdb, fdb, el_cfg, ner_cfg, ner_llm, args, write_lock: Lock):
    ...
    # NER for the next-hop candidates
    for hop, phrases in next_subjects_by_hop.items():
        ...
        while i < len(phrases):
            ...
            def _ner_call(**kwargs):
                return ner_llm(ner_messages, json_schema=ner_schema)

            out = _llm_call_with_retries(
                _ner_call,
...
def _openai_batch_loop(args, paths, qdb, fdb, el_cfg, ner_cfg, ner_llm):
    client = OpenAI(timeout=args.http_timeout, max_retries=args.http_retries)
    write_lock = Lock()
    ...
    while True:
        ...
        if not has_inflight and not has_pending and not has_to_parse:
            if t == 0:
                _dbg("[crawler] nothing to do.")
            else:
                _dbg(f"[crawler] queue drained: done={d} working={w} pending={p} total={t}")
            break
...
def _process_one(args, paths, fdb, el_cfg, ner_cfg, el_llm, ner_llm, subject, hop, write_lock: Lock):
    ...
    def _el_call(**kwargs):
        return el_llm(el_messages, json_schema=el_schema)

    resp = _llm_call_with_retries(
        _el_call,
        max_retries=args.http_retries,
        timeout=args.http_timeout,
        backoff=args.http_backoff,
...
def main():
    ap = argparse.ArgumentParser(description="Concurrent crawler (topic/general) with OpenAI Batch and threaded modes.")
    ...
    ap.add_argument("--openai-batch", action="store_true", help="Use OpenAI Batch API for elicitation when provider is OpenAI.")
    ap.add_argument("--openai-batch-size", type=int, default=10, help="Max subjects per OpenAI batch job.")
    ap.add_argument("--openai-batch-queue", type=int, default=4, help="Max number of outstanding OpenAI batches.")
    ap.add_argument("--openai-batch-window", default="24h", help="OpenAI batch completion window (e.g., 24h).")
    ap.add_argument("--openai-batch-poll", type=int, default=15, help="Seconds between batch status polls.")

    # Networking / reliability (NEW)
    ap.add_argument("--http-timeout", type=float, default=60.0, help="Per-request HTTP timeout in seconds.")
    ap.add_argument("--http-retries", type=int, default=6, help="Max HTTP retries for LLM/API calls.")
    ap.add_argument("--http-backoff", type=float, default=0.5, help="Base backoff (seconds) for retry sleep; exponential.")
    ...
    _dbg(f"[crawler] output_dir: {out_dir}")
    ...
    if args.resume and queue_has_rows(qdb):
        if args.reset_working:
            ...
            _dbg(f"[crawler] resume: reset {n} 'working' → 'pending'")
        d0, w0, p0, t0 = _counts(qdb, args.max_depth)
        _dbg(f"[crawler] resume: queue found: done={d0} working={w0} pending={p0} total={t0}")
    else:
        enqueue_subjects(qdb, [(args.seed, 0)])
        _dbg(f"[crawler] seeded: {args.seed}")
    ...
    for cfg in (el_cfg, ner_cfg):
        ...
        # network knobs (best-effort – adapters may ignore some)
        try: cfg.request_timeout = args.http_timeout
        except Exception: pass
        try: cfg.max_retries = args.http_retries
        except Exception: pass
        try: cfg.retry_backoff = args.http_backoff
        except Exception: pass
        ...
        with ThreadPoolExecutor(max_workers=max(1, args.concurrency)) as pool:
            while True:
                if args.max_subjects and subjects_elicited_total >= args.max_subjects:
                    _dbg(f"[crawler] max-subjects reached ({subjects_elicited_total}); stopping.")
                    break
                ...
                for fut in as_completed(fut_map):
                    ...
                    try:
                        ...
                        n = reset_working_to_pending(qdb)
                        print(f"\n[crawler] Interrupted. reset {n} 'working' → 'pending' for resume.")
                        raise
                    except Exception:
    ...
    dur = time.time() - start
    print(f"[crawler] finished in {dur:.1f}s → outputs in {out_dir}")
    print(f"[crawler] queue.json        : {paths['queue_json']}")
    print(f"[crawler] facts.json        : {paths['facts_json']}")
    print(f"[crawler] facts.jsonl       : {paths['facts_jsonl']}")
    print(f"[crawler] lowconf.json      : {paths['lowconf_json']}")
    print(f"[crawler] lowconf.jsonl     : {paths['lowconf_jsonl']}")
    print(f"[crawler] ner log           : {paths['ner_jsonl']}")
    print(f"[crawler] errors.log        : {paths['errors_log']}")
    print(f"[crawler] batch_state       : {paths['batch_state']}")
    print(f"[crawler] batches dir       : {paths['batches_dir']}")
